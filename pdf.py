"""Render an onboarding record into a Private Pricing Agreement PDF.
...
Layout:
  1. Parties (both names + representatives)
  2. Pricing Schedule -- a two-column key/value table
  3. Terms and Conditions -- boilerplate clauses
  4. Signatures
"""
...
def build_invoice_pdf(invoice: dict) -> bytes:
    ...
    pdf.set_text_color(15)
    pdf.cell(0, 24, "Invoice", new_x="LMARGIN", new_y="NEXT")
    ...
    meta = [
        ...
        f"Billing period:  {inv['billing_period']}    Issued (UTC):  {inv.get('issued_at') or '-'}",
        f"Currency:  {inv['currency']}    Price book:  {inv['price_book_date']}    Terms:  net 30, in arrears",
    ]
    for line in meta:
    ...
    gross = inv["gross_subtotal"] or 1
    total_row("Gross subtotal", _amt(inv["gross_subtotal"]))
    total_row(f"Discount ({inv['discount_total'] / gross * 100:.0f}%)", f"-{_amt(inv['discount_total'])}")
    total_row("Net total due", _amt(inv["total"]), bold=True)

    pdf.set_auto_page_break(False)
    pdf.set_y(-30)
    pdf.set_font("Helvetica", "I", 8)
    pdf.set_text_color(140)
    pdf.cell(0, 10, f"{inv['invoice_number']} - generated by Private Pricing Studio")
